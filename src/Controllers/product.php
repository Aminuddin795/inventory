...
// code to adding or insert a new item

$app->post('/products/add', function ($request,$response,$args ) {
  $form_data = $request->getParsedBody();
  $data = createProduct($this->db,$form_data);
  if ($data <=0) {
    return $this->response->withJson(array('error' => 'fail to add data'), 500);
  }
  return $this->response->withJson(array('add data' => 'success add new data'), 201);
}
);

  //code to update a item on database
